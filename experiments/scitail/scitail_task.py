...
@register_task(name="SciTail", desc="An entailment dataset created from multiple-choice science exams and web sentences: https://allenai.org/data/scitail")
class SciTailTask(Task):
  ...
  def train_data(self, max_seq_len=512, dataset_size=None, epochs=1, mask_gen=None, **kwargs):
    ...
    examples = [ExampleInstance((l[0],l[1]), self.label2id(l[2])) for l in data]
...
dataset_size = dataset_size, shuffle=True, **kwargs)
  ...
  def _data(self, name, path, type_name = 'dev'):
    ...
    data = self._read_tsv(input_src)
    if type_name=='test':
      examples = ExampleSet([ExampleInstance((l[0], l[1])) for l in data])
    else:
      examples = ExampleSet([ExampleInstance((l[0],l[1]), self.label2id(l[2])) for l in data])
